...
t_step = 0.01
t = np.arange(0.0, 80.0, t_step)    #from 0 to 20 seconds with increments of each step as 0.01
...
class Drone:
  ...
  def Control(self, sp):
    ...
    resp = self.controller.Respond(pv, sp)
...
class PID_Controller():
  def __init__(self, kp, ki, kd):
    self.kp = kp  #PID controller constants
    self.ki = ki
    self.kd = kd
    self.E = 0  #set initial error as 0
    self.I = 0  #set initial integral term as 0
    self.sp = 0 #set initial setpoint as 0

  def Respond(self, pv, sp):
    if (sp != self.sp): #if new setpoint given
      self.I = 0  #reset integral
      self.sp = sp  #renew setpoint
    err = sp - pv #calculate error
    P = self.kp * err #calculate PID terms
    I = self.I + self.ki * err * t_step
    D = self.kd * (err - self.E)/t_step
    self.E = err  #store error as attribute
    self.I = I  #store integral term as attribute
    return P + I + D  #return aggregated PID sum

#MAIN
ki_limit = 800
#initialise drones
drone1 = Drone(-9.81, 0.25, 0.9, 0.03, 0.1, 10, 1000, PID_Controller(600, 10, 20))
drone2 = Drone(-9.81, 0.25, 0.9, 0.03, 0.1, 10, 1000, PID_Controller(600, 10, 80))
drone3 = Drone(-9.81, 0.25, 0.9, 0.03, 0.1, 10, 1000, PID_Controller(600, 10, 400))
drone4 = Drone(-9.81, 0.25, 0.9, 0.03, 0.1, 10, 1000, PID_Controller(600, 10, 3000))
...
ax1.plot(t, drone1.pos, label = 'Kp = 600, Ki = 10, Kd = 20')
ax1.plot(t, signal, label = 'Signal')
ax2.plot(t, drone2.pos, label = 'Kp = 600, Ki = 10, Kd = 80')
ax2.plot(t, signal, label = 'Signal')
ax3.plot(t, drone3.pos, label = 'Kp = 600, Ki = 10, Kd = 400')
ax3.plot(t, signal, label = 'Signal')
ax4.plot(t, drone4.pos, label = 'Kp = 600, Ki = 10, Kd = 3000')
