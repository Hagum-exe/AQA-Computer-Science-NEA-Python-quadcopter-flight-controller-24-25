...
df = pd.read_csv('vert_velo_throttle_IO_test.csv')
print(df.head)
index = list(df.index.values)

throttle_input_list = df['throttle_input'].values.tolist()
...
axis[0].set_title('throttle_input')
...
axis[0].plot(index, throttle_input_list, color = 'orange')
