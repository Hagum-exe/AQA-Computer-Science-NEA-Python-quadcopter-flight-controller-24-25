...
df = pd.read_csv('rate_mpu_IO_test.csv')
print(df.head)
index = list(df.index.values)
mpu_pitch_list = df['mpu_pitch'].values.tolist()
mpu_roll_list = df['mpu_roll'].values.tolist()
mpu_yaw_list = df['mpu_yaw'].values.tolist()
...
axis[0].plot(index, mpu_yaw_list, color = 'orange')
