...
angle_i_term_limit = 0  #I-term limit to prevent integral windup
...
def run():   
  ...
  if mpu_online and rx_online and nano_online and bmp_online:   #if all necessary devices are online
    ...
    f = open('rate_pid_rx_input_test.csv', 'w')
    f2 = open('rate_motor_IO_test.csv', 'w')
    ...
    while True:   #flight control loop executes forever until power is disconnected
        ...
        ############################### ARMED, AIR MODE ###############################  
            ...
            #msg = f'{pitch_input},{roll_input},{yaw_input},{roll_resp},{pitch_resp},{yaw_resp}'
            ...
            msg = f'{pitch_input},{roll_input},{yaw_input},{pitch_resp},{roll_resp},{yaw_resp},{t1},{t2},{t3},{t4}'
            f2.write(f'{msg}\n')
            #print(f'{t1}, {t2}, {t3}, {t4}')
